...
class UpdateDeliverySpeedRequest extends FormRequest
{
    ...
    {
        $deliverySpeedId = $this->route('delivery_speed')?->id;
        ...
        foreach (crudLocaleCodes() as $locale) {
            $rules["translations.{$locale}.name"] = [
                ...
                'string',
                'max:120',
                Rule::unique('delivery_speed_translations', 'name')
                    ->where('locale', $locale)
                    ->ignore($deliverySpeedId, 'delivery_speed_id'),
            ];

...
<div class="card mb-4">
    <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
        <div>
            <h5 class="mb-1">{!! __('general.menu_pricing_rules') !!}</h5>
        </div>
        <div class="d-flex gap-2">
            @can('pricing_rules.create')
            <a href="{{ route('admin.pricing-rules.create') }}" class="btn btn-primary">{!! __('general.add_pricing_rule') !!}</a>
            @endcan
        </div>
    </div>
    ...
    <div class="card-datatable text-nowrap">
        <table class="datatables-records table">
            ...
            <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-inline-block">
                                @can('pricing_rules.edit')
                                <a href="{{ route('admin.pricing-rules.edit', $rule) }}" class="text-body"><i class="text-primary ti ti-pencil"></i></a>
                                @endcan
                                @can('pricing_rules.delete')
                                <a href="javascript:;" onclick="deleteConfirmation({{ $rule->id }})" class="text-danger"><i class="ti ti-trash"></i></a>
                                <form action="{{ route('admin.pricing-rules.destroy', $rule) }}" method="POST" id="deleteForm{{ $rule->id }}">
                                    @csrf
                                    @method('DELETE')
                                </form>
                                @endcan
                ...
                @empty
                    <tr>
                        <td colspan="7" class="text-center text-muted">{{ __('general.no_pricing_rules_yet') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
<script>
    ...
    $(document).on('click', '.changeStatus', function () {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: "{!! route('admin.pricing-rules.update-status') !!}",
                    type: 'POST',

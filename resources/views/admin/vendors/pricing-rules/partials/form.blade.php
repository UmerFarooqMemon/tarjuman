@php
    ...
    $selectedPair = old('vendor_language_pair_id', optional($rule)->vendor_language_pair_id);
@endphp

@if ($pairs->isEmpty())
<div class="col-12 mb-3">
    <div class="alert alert-warning mb-0">
        {!! __('general.create_language_pair_before_pricing') !!}
        <a href="{{ route('admin.vendors.language-pairs.create', $vendor) }}">{!! __('general.add_language_pair') !!}</a>
    </div>
</div>
@endif

<div class="mb-3 col-md-6">
    <label class="form-label" for="vendor_language_pair_id">{!! __('general.language_pair') !!} <span class="required-fl">*</span></label>
    <select class="form-select" id="vendor_language_pair_id" name="vendor_language_pair_id" required @disabled($pairs->isEmpty())>
        <option value="">{{ __('general.select') }}</option>
        @foreach ($pairs as $pair)
            <option value="{{ $pair->id }}" @selected((string) $selectedPair === (string) $pair->id)>
                {{ $pair->label() }}
            </option>
        @endforeach
    </select>
...
<div class="mb-3 col-md-3">
    ...
    <div class="input-group">
        ...
        <span class="input-group-text d-inline-flex align-items-center">
            ...
            <span>{{ old('currency', optional($rule)->currency ?? platformCurrency()) }}</span>
        </span>
    </div>
